...
def run_using_pes_val(df: pd.DataFrame):
    # This will drop the columns that you don't want to target yet
    tmp = df[[col for col in df.columns if col not in [
        'is_pes', 'is_very_pes', 'is_very_opt', 'attitude', 'avg_family']]]

    # Create instance
    o = Attitudes_explore(tmp, target='pes_val', subset_title = 'Whole dataset with top 5 lowest p-vals using pes_val')
